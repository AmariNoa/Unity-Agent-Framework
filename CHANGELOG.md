...
- Self-healing watchdog in AgentCoreBootstrap that restarts the agent HTTP server when it is found down while the editor is idle (recovers from a delayCall that did not run, e.g. while the editor window was unfocused), with exponential backoff between retries (2s doubling up to 60s) and automatic retries stopped after 10 consecutive failures within the same domain, all recorded to bootstrap.log
- "Tools/Unity Agent Framework/Restart Agent Server" menu item to manually stop and restart the agent HTTP server and clear any watchdog give-up state
